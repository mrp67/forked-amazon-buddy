...
export class AmazonScraper {
    ...
    extractProductInfromation($) {
        ...
        try {
            ...
            for (let item in bestSalesSection) {
                ...
                console.log('bestSalesSection[item].children[1].children[1]: ', bestSalesSection[item].children[1].children[1]);
                ...
                if (item.type === 'tag' && item.name === 'ul') {
                    if (item.children) {
                        item.children.forEach((rankItem) => {
                            ...
                            console.log('rankItem.children[0].children[1].attribs: ', rankItem.children[0].children[1].attribs);
                            if (category && rank) {
                                const bestSeller = this.geo.best_seller(`${rank} ${category}`);
                                if (bestSeller) {
                                    bestsellers_rank.push({ ...bestSeller, link: `${this.mainHost}${link}` });
    ...
    grabProduct(body, p) {
        ...
        for (let i = 0; i < productList.length; i++) {
            ...
            const href = $('div div div div div div span a').attr('href');
            let hrefLink = href?.match('(url=).+')[0] || ''
            hrefLink = hrefLink.replace('url=', '');
            ...
            scrapingResult[asin] = {
                ...
                link: `https://amazon.com${hrefLink}`,
